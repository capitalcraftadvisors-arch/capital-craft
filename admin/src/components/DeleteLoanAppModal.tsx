...
// Type-to-confirm modal for permanently deleting a loan application.
//
// Mirrors DeleteEpcModal: shows the applicant + display id, requires the
// admin to type "DELETE", then POSTs /api/admin/delete-loan-app. On
// success → onDeleted() (parent navigates back to the applications list).
...
export default function DeleteLoanAppModal({
  open, onClose, onDeleted,
  applicationId, displayId, applicant, mobile,
}: Props) {
  ...
  async function doDelete() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/delete-loan-app", {
        method: "POST",
        headers: {
      ...
      >
        <div className="bg-red-50 border-b border-red-100 px-6 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="font-display font-bold text-[17px] text-red-700">
              Delete loan application permanently
            </h2>
          </div>
        </div>
        ...
        <div className="p-6 space-y-4">
          <p className="text-[13px] text-text">
            You&rsquo;re about to permanently delete this loan application and
          ...
          <div className="rounded-input border border-line bg-bg-tint p-4 text-[13px] space-y-1.5">
            <div className="flex gap-3">
              <span className="text-text-muted min-w-[80px]">Loan ID</span>
          ...
          <div className="text-[12px] text-text-muted">
            ...
            <ul className="list-disc pl-5 space-y-0.5">
              <li>The loan application row</li>
              <li>All uploaded documents (PAN, Aadhaar, e-bill, photos, quotation, etc.)</li>
        ...
        <div className="border-t border-line px-6 py-4 flex justify-end gap-2">
          ...
          >
            {busy ? "Deleting…" : "Delete application permanently"}
          </button>
        </div>
      </div>
    </div>
