...
//   kind = "plant" → geo-tagged plant photo       → plant_photo_path + plant_photo_gps
//
// The client captures GPS for the plant photo (EXIF or live geolocation, same
// as the office/rooftop photos) and passes it as a JSON string; we persist it
// as-is. Image compression happens here, AFTER the client has already read the
// GPS off the original file — so nothing is stripped before capture.
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (kind !== "gst" && kind !== "plant") return err("Invalid upload kind.", 400);
    ...
    const folder = kind === "gst" ? "gst" : "plant";
    const path = `insurance/${id}/${folder}/${Date.now()}_${safeName(file.name, folder)}`;
    await uploadBuffer(path, buf, mime);

    const patch: Record<string, unknown> = kind === "gst"
      ? { gst_path: path }
      : { plant_photo_path: path, plant_photo_gps: gps };
